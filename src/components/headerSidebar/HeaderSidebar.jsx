...
const HeaderSidebar = ({ activeMenu }) => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className="w-1/5 flex flex-col h-[74%] bg-white shadow-md rounded-md justify-between fixed mt-40">
                <div className="mt-4">
                    {menuItems.map((item, index) => (
                        <div
                            key={index}
                            className={`flex items-center gap-6 px-16 py-2 text-lg ${activeMenu === item.label
